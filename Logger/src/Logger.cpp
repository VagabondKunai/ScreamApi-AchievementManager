...
void log(LogLevel level, const char* const message, va_list args){
	...
	{ // Code block for lock_guard destructor to release lock
		...
		char* logLine = new char[4096 + 128];
		sprintf_s(logLine, 4096 + 128, "[%s] [%s]\t%s\n", timeStr, strLogLevels[(int)level], buffer);

		// Use Windows API for guaranteed write
		HANDLE hFile = CreateFileW(logFilepath.c_str(), FILE_APPEND_DATA, FILE_SHARE_READ, 
		                           NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if(hFile != INVALID_HANDLE_VALUE) {
			DWORD written = 0;
			WriteFile(hFile, logLine, (DWORD)strlen(logLine), &written, NULL);
...
void Logger::flush() {
	// This is a no-op since files are flushed on each write,
	// but having it here ensures intentional flushes on shutdown
}